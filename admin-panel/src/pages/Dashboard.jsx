...
const Dashboard = () => {
  ...
  const resetFields = () => {
    setLevelForm({ title: '', description: '', xpReward: 50, hasGame: false, hasQuiz: false, isPublished: false, theory: [{ type: 'paragraph', content: '' }] });
    setQuizForm({ questions: [{ question: '', options: ['', '', '', ''], correctAnswer: '' }], passingScore: 70 });
    setGameForm({ name: '', type: 'logic', difficulty: 'easy', instructions: '', xpReward: 50 });
    setFormTab('theory');
  };

  const addTheoryBlock = () => setLevelForm({ ...levelForm, theory: [...levelForm.theory, { type: 'paragraph', content: '' }] });
  const updateTheoryBlock = (idx, field, value) => { const n = [...levelForm.theory]; n[idx][field] = value; setLevelForm({ ...levelForm, theory: n }); };
  const removeTheoryBlock = (idx) => setLevelForm({ ...levelForm, theory: levelForm.theory.filter((_, i) => i !== idx) });
                      ...
                      <div className='space-y-4 md:space-y-6 max-w-2xl'>
                        ...
                        <div className='space-y-2 md:space-y-3'>
                          <div className='flex justify-between items-center'>
                            <span className='text-[8px] md:text-[9px] font-bold text-zinc-700 uppercase'>Content</span>
                            <button onClick={addTheoryBlock} className='text-[8px] md:text-[9px] font-bold bg-zinc-800 hover:bg-zinc-700 px-2 md:px-3 py-1 md:py-1.5 rounded transition-all cursor-pointer'>+ Add</button>
                          </div>
                          {levelForm.theory.map((b, i) => (
                            <div key={i} className='flex gap-1 md:gap-2'>
                              <select value={b.type} onChange={e => updateTheoryBlock(i, 'type', e.target.value)} className='bg-zinc-950 border border-zinc-900 text-[8px] md:text-[9px] font-bold uppercase p-1.5 md:p-2 rounded outline-none'>
                                <option value='paragraph'>P</option>
                                <option value='bullet'>B</option>
                              </select>
                              <textarea value={b.content} onChange={e => updateTheoryBlock(i, 'content', e.target.value)} className='flex-1 bg-zinc-950 border border-zinc-900 p-2 md:p-3 rounded text-xs md:text-sm h-16 md:h-20 outline-none' placeholder='Content...' />
                              <button onClick={() => removeTheoryBlock(i)} className='text-zinc-800 hover:text-red-500 transition-colors cursor-pointer'><Trash2 size={12} className='md:w-3.5 md:h-3.5' /></button>
                            </div>
                          ))}
                        </div>
