...
const VoiceAssistant = ({ context }) => {
    ...
    const handleAiProcess = async (userText) => {
        ...
        try {
            ...
            // Format context if it exists (it's an array of objects)
            const contextText = context ? context.map(item => item.content).join("\n") : "";
                ...
                <div className="px-5 py-4 border-b border-white/5 flex justify-between items-center bg-white/5">
                    <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full shadow-[0_0_10px_currentColor] ${
                             status === 'listening' ? 'bg-red-500 text-red-500 animate-pulse' : 
